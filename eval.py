...
import numpy as np
import PostProcessModule as postProcess
...
tf.flags.DEFINE_string("inf_dir", "inf_res/", "path to save inference result")
...
def main(argv=None):
    ...
    if FLAGS.mode == "visualize":
        max_out_itr=validation_dataset_reader.images.shape[0]/FLAGS.batch_size
        for out_itr in range(max_out_itr):
            ...
            for itr in range(FLAGS.batch_size):
                utils.save_image(valid_images[itr].astype(np.uint8), FLAGS.inf_dir, name="inp_" + str(out_itr) + "_" + str(5+itr))
